...
# Usage:  sudo bash run_all_attacks.sh
#
# Before running:
#   1. Start benign traffic on Ubuntu Server:
#      bash run_benign_all.sh
#   2. Then run this script on Kali
#   3. When done, stop benign traffic:
#      bash stop_benign_all.sh
# ============================================================

if [ "$EUID" -ne 0 ]; then
    echo "[ERROR] Must run as root: sudo bash run_all_attacks.sh"
    exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SESSION_LOG="/tmp/session_log_entries.csv"
...
echo " All results logged to: ${SESSION_LOG}"
echo "============================================"
echo ""
...
bash "${SCRIPT_DIR}/nmap_syn_scan.sh"
...
echo "============================================"
START=$(date '+%Y-%m-%d %H:%M:%S')
bash "${SCRIPT_DIR}/nmap_service_scan.sh"
END=$(date '+%Y-%m-%d %H:%M:%S')
...
echo "============================================"
START=$(date '+%Y-%m-%d %H:%M:%S')
bash "${SCRIPT_DIR}/nmap_evasion_scan.sh"
END=$(date '+%Y-%m-%d %H:%M:%S')
...
echo "============================================"
START=$(date '+%Y-%m-%d %H:%M:%S')
bash "${SCRIPT_DIR}/hydra_ssh_brute.sh"
END=$(date '+%Y-%m-%d %H:%M:%S')
...
echo "============================================"
START=$(date '+%Y-%m-%d %H:%M:%S')
bash "${SCRIPT_DIR}/hydra_http_brute.sh"
END=$(date '+%Y-%m-%d %H:%M:%S')
...
echo "============================================"
START=$(date '+%Y-%m-%d %H:%M:%S')
python3 "${SCRIPT_DIR}/c2_beacon.py" --mode all --duration 120 --interval 15
END=$(date '+%Y-%m-%d %H:%M:%S')
...
echo "============================================"
START=$(date '+%Y-%m-%d %H:%M:%S')
bash "${SCRIPT_DIR}/slowhttptest_dos.sh"
END=$(date '+%Y-%m-%d %H:%M:%S')
...
echo "============================================"
START=$(date '+%Y-%m-%d %H:%M:%S')
bash "${SCRIPT_DIR}/metasploit_ms17010.sh"
END=$(date '+%Y-%m-%d %H:%M:%S')
...
echo ""
echo "NEXT STEPS:"
echo "  1. Copy ${SESSION_LOG} contents into"
echo "     data/session_log.csv on Ubuntu Server"
echo "  2. Stop benign traffic:"
echo "     bash stop_benign_all.sh"
echo "  3. Run pipeline on Ubuntu Server:"
echo "     bash pipeline/sync_pcaps.sh"
echo "     bash pipeline/run_cicflowmeter.sh"
echo "     python3 pipeline/label_flows.py"
echo ""
cat "$SESSION_LOG"
